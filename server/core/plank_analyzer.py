"""Plank Analyzer - Core stability tracking"""
import cv2
...
def get_plank_metrics(landmarks, width, height):
    ...
    try:
        ...
        # Body alignment angle (should be close to 180 for straight plank)
        metrics['body_alignment'] = calculate_angle(shoulder_mid, hip_mid, ankle_mid)
        
        # Hip sag detection - measure distance from hip to line between shoulder and ankle
        ...
        # Positive if hip is below line (sagging), negative if above (piking)
        # Use y-coordinate comparison
        ...
        if hip_mid[1] > expected_y:
            metrics['hip_position'] = int(hip_deviation)  # Sagging (positive)
        else:
            metrics['hip_position'] = -int(hip_deviation)  # Piking (negative)
...
def get_form_rating(body_alignment, hip_position):
    ...
    # Ideal plank: body_alignment close to 180, hip_position close to 0
...
def add_metric_overlays(image, metrics, elapsed_time):
    ...
    if metrics['body_alignment'] is not None:
        if 165 <= metrics['body_alignment'] <= 195:
            color = (0, 255, 0)  # Green - good alignment
        else:
            color = (0, 165, 255)  # Orange - needs adjustment
    ...
    if metrics['hip_position'] is not None:
        if abs(metrics['hip_position']) < 15:
            ...
            hip_color = (0, 0, 255)  # Red - sagging
            status = "Sagging"
        else:
            hip_color = (255, 165, 0)  # Orange - piking
            status = "Piking"
...
def analyze_plank_video(video_path, output_path=None):
    ...
    with mp_pose.Pose(min_detection_confidence=0.7, min_tracking_confidence=0.7, model_complexity=1) as pose:
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                # Detect plank position (horizontal body)
                if metrics['body_alignment'] is not None:
    ...
    # Add final plank time if still in plank at end
    if in_plank:
        total_plank_time += (frame_count - plank_start_frame) / fps
