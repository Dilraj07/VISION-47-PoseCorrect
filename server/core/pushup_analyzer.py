...
def add_info_panel(image, frame, total_frames, fps, reps, current_elbow_angle, hip_drop, velocity_str="--"):
    h, w = image.shape[:2]
    ...
    if velocity_str != "--":
        cv2.putText(image, f"Vel: {velocity_str} d/s", (w - 200, h - 55), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1, cv2.LINE_AA)

    status_text = "GOOD FORM" if (current_elbow_angle <= 100 and hip_drop < 20) else "ADJUST FORM"
    status_color = NEON_GREEN if status_text == "GOOD FORM" else (147, 20, 255)
...
def analyze_pushup_video(video_path, output_path=None):
    ...
    last_good_metrics = {k: None for k in ['left_elbow', 'right_elbow', 'shoulder_angle', 'hip_drop', 'torso_angle']}
    ...
    with mp_pose.Pose(min_detection_confidence=0.7, min_tracking_confidence=0.7, model_complexity=1) as pose:
        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                if metrics['left_elbow'] is not None:
                    # ===== Velocity tracking =====
                    vel_data = velocity_tracker.update(metrics['left_elbow'])
            ...
            final_image = add_info_panel(image_rgb, frame_count, total_frames, fps, rep_count, min_elbow_angle, max_hip_drop, last_velocity_str)
