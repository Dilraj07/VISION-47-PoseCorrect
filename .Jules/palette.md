...
## 2024-07-22 - [Keyboard Focus Parity via State] **Learning:** Attempting to simulate hover states on interactive UI elements (like custom `div` or `Link` components) using direct DOM manipulation (`e.currentTarget.style`) in `onMouseEnter` prevents those same styles from being easily applied to `onFocus` for keyboard users, leading to a loss of accessibility context. **Action:** Always use React state (`useState`) to track an `isHovered` boolean that handles both `onMouseEnter/Leave` and `onFocus/Blur` handlers to ensure perfect visual parity between mouse and keyboard navigation.
