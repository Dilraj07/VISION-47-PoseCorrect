...
## 2025-08-02 - Replace Direct DOM Manipulation with Declarative Animations\n**Learning:** Found several instances where `onMouseEnter` and `onMouseLeave` were used to directly manipulate the DOM styles of elements (like changing border color or text color) to simulate hover effects. This approach completely ignores keyboard users, as the effects aren't triggered on focus. Moreover, directly changing styles breaks React's declarative paradigm and can lead to out-of-sync visual states.\n**Action:** Always use declarative animation libraries like Framer Motion (`whileHover` and `whileFocus`) or React state (to track `isFocused` or `isHovered`) to manage interactive states. This ensures that hover and focus states are visually identical and robustly managed by React.
