...
## 2024-05-18 - [Fix React Conditional Rendering Bug in Empty States] **Learning:** In React JSX, complex boolean conditions wrapped in `{}` need to be properly parenthesized if they involve logical OR `||` followed by a logical AND `&&`, otherwise the whole conditional block might evaluate incorrectly and fail to render entirely, leaving a true/false value in the DOM. **Action:** Always wrap `A || B` conditions in parenthesis like `(A || B) && <Component />` to ensure correct rendering evaluation.
