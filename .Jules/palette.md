...
## 2024-05-15 - [Add ARIA labels to icon-only buttons] **Learning:** [Icon-only links implemented using arrays mapped over components need accessible labels directly injected as properties so that screen readers can effectively parse the buttons.] **Action:** [Always check mapping iterations in React code to verify that aria-label properties are set when rendering icon-only interactable elements.]
