...
## 2026-06-20 - [Keyboard Accessibility for Interactive Divs] **Learning:** When building or modifying interactive `div` elements that function as buttons or cards (like the ExerciseStrip), they require explicit keyboard accessibility support. This means adding `role="button"`, `tabIndex`, `onKeyDown` (for Enter/Space), and `onFocus`/`onBlur` handlers to replicate hover visual states. **Action:** Always add keyboard event handlers and ARIA attributes when repurposing non-interactive elements into actionable components to ensure screen reader and keyboard-only users can navigate the interface effectively.
