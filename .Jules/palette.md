...
## 2026-06-27 - [Interactive Div Accessibility] **Learning:** When building interactive `div` elements that act as buttons or cards (e.g., ExerciseStrip, ModeSelection), adding `onFocus`/`onBlur` handlers to replicate the visual states of `onMouseEnter`/`onMouseLeave` is critical for ensuring keyboard navigation users get the same visual feedback as mouse users. **Action:** Always pair hover state handlers with focus state handlers, and ensure `role="button"`, `tabIndex`, and `onKeyDown` (for Enter/Space) are also present for full keyboard a11y.
