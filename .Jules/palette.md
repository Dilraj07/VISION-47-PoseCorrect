...
## 2024-05-18 - [Navbar Accessibility] **Learning:** Defining components inside other components (like `NavItem` inside `Navbar`) causes unnecessary re-renders. Also, direct DOM manipulation for styling (`e.currentTarget.style`) makes it harder to support both mouse and keyboard interactions natively. **Action:** Extract sub-components outside the parent scope and use React state (`useState`) for `onMouseEnter`/`onMouseLeave`/`onFocus`/`onBlur` to maintain accessible visual parity.
